...
uninstallcronjob() {
  ...
  if [ "$cr" ]; then
    if _exists uname && uname -a | grep solaris >/dev/null; then
      ...
      $_CRONTAB -l | sed "/install-cert/d" | $_CRONTAB --
    else
      $_CRONTAB -l | sed "/$PROJECT_ENTRY --cron/d" | $_CRONTAB -
      $_CRONTAB -l | sed "/install-cert/d" | $_CRONTAB -
    fi
...
_installalias() {
  ...
  if [ "$_profile" ]; then
    ...
    _info "OK, Close and reopen your terminal to start using enca"
  else
...
uninstall() {
  ...
  rm -rf "$LE_WORKING_DIR"
  __green "uninstall finished"
  echo ""
...
_uninstallalias() {
  ...
  cat ~/.bashrc | sed -i "/export nginx_path/d"  ~/.bashrc
  cat ~/.bashrc | sed -i "/export domain/d" ~/.bashrc
  cat ~/.bashrc | sed -i "/alias enca/d" ~/.bashrc
  _profile="$(_detect_profile)"
  if [ "$_profile" ]; then
...
_installOnline() {
  ...
  if [ -f "$DIR/enca.tar.gz" ]; then
    _info "Found enca pkg,use enca.tar.gz"
    #statements
    localname="enca.tar.gz"
  else
